...
Zeta:
  Yes. Looking across the project, **Sigil is a terminal-native LLM assistant/runtime** built around zsh punctuation shortcuts and a Python CLI.
  
  At a high level, it does four things:
  
  1. **Shell UX**
     - Provides zsh bindings in `src/sigil/bindings/sigil.zsh`.
  ...
  2. **Python CLI/runtime**
     - The installed command is `sigil`, defined in `pyproject.toml`.
     - CLI modules live under `src/sigil/cli/`: install, run, status, session, log, trace, step, model, events.
  ...
  3. **Agent + tool loop**
     - `src/sigil/zeta/` contains the embedded agent runtime, called **Zeta**.
     ...
     - Tool implementations include read/list/grep/write/edit/bash/query-log style operations under `src/sigil/tools/` and `src/sigil/zeta/tools/`.
  
  4. **Durable session/state/ledger system**
     - `src/sigil/state.py`, `session.py`, `ledger.py`, `events.py`, and `protocols.py` define persistent records for turns, effects, shell handoffs, failures, and outcomes.
